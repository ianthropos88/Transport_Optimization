# Optimize Your Transportation with the Multimodal Concept using Python. 🚚
...
This solution uses mathematical programming to model situations, where, because of different choices of routes and transporation tools we have high level costs, and provides solution for overall cost minimization. The model construction offers options of two mathematical programming frameworks, **DOcplex** and **CVXPY**.
...
The data of all the above parameter matrices will be imported from **model data.xlsx** with function **transform()** and **set_param()**. For more details, please refer to the codes in [**multi_model_transpotation.py**](https://github.com/ianthropos88/transport_optimization/blob/main/multi-model%20transportation.py).
...
Finally, hope this long documentation helps.
